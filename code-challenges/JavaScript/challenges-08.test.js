...
Write a function named filterStringsWithVowels that, given an array of strings as 
input, uses filter to return an array with only words that contain vowels.
...
const filterStringsWithVowels = (arr) => arr.filter(i => i.match(/[aeiou]/ig));
...
const notInFirstArray = (forbiddenValues, arr) => arr.filter(i => !forbiddenValues.includes(i))
...
const getBaseStatGreaterThan = (arr, minBaseStat) => arr.filter(num => num.baseStat > minBaseStat);
...
const getStatName = (arr, minBaseStat) => arr.filter(i => i.baseStat > minBaseStat).map(i => i.stat.name);
...
const getCharactersWithoutChildren = (arr) => arr.filter(num => !num.hasOwnProperty('children'));
...
const evenOddNumericValues = (arr) => arr.filter(num => typeof(num) === 'number').map(num => (num % 2) ? 'odd' : 'even');
