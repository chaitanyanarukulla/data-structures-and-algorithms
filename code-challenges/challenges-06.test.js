...
challenge 3, but uses the data's entries instead of its values.
...
// const hasChildrenEntries = (arr, character) => {
//   arr.forEach(ele => {
//     if (arr[ele].name === character) {
//       return arr[ele].children.length > 0;
//     }
//   });
// };
...
const totalCharacters = (arr) => {
  ...
  arr.forEach(character => {
    total.push(character.name);
    if (character.spouse && !total.includes(character.spouse)) {
      total.push(character.spouse);
    }
...
const houseSize = (arr) => {
  ...
  arr.forEach(family => {
    let members = {
      house: family.house,
      members: ((family.spouse) ? 1 : 0) + Number(family.children.length) + 1,
    };
    sizes.push(members);
  });
  return sizes;
